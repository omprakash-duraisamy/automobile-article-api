...
exports.saveArticle = async (req, res) => {
    ...
    try {
        let body = req.body;
        let query = "insert into articles(title,manufacturer,url,image,thumbnail_image) values($1,$2,$3,$4,$5)";
...
exports.fetchArticle = async (req,res) => {
    ...
    let client = getDatabaseClient();
    try {
        let result = await client.query(`select * from articles where "articleID"=${articleID}`);
        res.send(result.rows);
    } catch (err) {
...
exports.deleteArticle = async (req,res) => {
    ...
    let client = getDatabaseClient();
    try {
        await client.query(`delete from articles where "articleID"=${articleID}`);
        res.end();
    } catch (err) {
